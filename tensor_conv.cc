...
int main(int argc, char* argv[]) {
    ...
    if (argc != 3) {
    cerr << "Usage:  " << argv[0] << " input_tensorproto_file output_file" << endl;
    return -1;
    }
    ...
    conv_tensor_init(tensor);

    {
        fstream output(argv[2], ios::out | ios::trunc | ios::binary);
        if (!tensor.SerializeToOstream(&output)) {
    ...
    cout << "TensorProto successfully converted.."<< endl;
    return 0;
}
